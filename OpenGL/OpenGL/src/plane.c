...
_Out_	PGlusLine	_l)
{
	...
	GLdouble	r = glusDotPro(&vn, &vba);
	if (r < Glus_Zero)
		return Intersect_Parallel;
	else
	{
		GlusVector	v = glusVector3fv((&_pa->A), (&_pb->A));
		GLdouble	e = glusDotPro(&vn, &v) / r,
					t = glusDotPro(&vn, &vbb) / r;

		// so we get a point on line
		glusAdd(&_pb->A, 1, &vba, e, &_l->A); 

		// and get the vector for line
		glusAdd(&vbb, 1, &vba, -t, &_l->B);

		// so convert the form to point
		glusAdd(&_l->A, 1, &_l->B, 1, &_l->B);
