...
_In_	Glusnum		_n_stack)
{
	...
	glusAllocex(_mesh->Faces, GlusFace, _n_piece*_n_stack, return Glus_Status_Memory_Allocate_Fail);
...
#define FN(n)		F.FaceIDs[n].NormalID
	for (Glusnum v = 0; v < _n_stack; v++)
	{
		for (Glusnum u = 0; u < _n_piece; u++, _mesh->FaceNum++)
		{
			...
			F.FaceIDNum = 4; glusAllocex(F.FaceIDs, GlusFaceIndex, 4, return Glus_Status_Memory_Allocate_Fail);
